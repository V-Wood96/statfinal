...
The Analysis of Deviance Table shows that ASSET_AMT and REVENUE_AMT are by far the strongest predictors, followed by the organization’s fiscal year-end month (ACCT_PD), NTEE_major, State and Group status. Group status was the only insignificant predictor. We used a generalized linear model with the predictors to test the accuracy of the model we set up a 80/20 train/test split which had a 0.97 accuracy.
